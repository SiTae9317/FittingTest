...
public class MeshSplit : MonoBehaviour 
{
	...
	{
		using (FileStream fs = new FileStream ("C://RigData//newClosedRigData.bytes", FileMode.OpenOrCreate, FileAccess.Write)) 
		{
	...
	void importBinaryCRD()
	{
		...
		TextAsset ta = Resources.Load ("newClosedRigData") as TextAsset;
	...
	{
		using (FileStream fs = new FileStream ("C://RigData//newStandardJointData.bytes", FileMode.OpenOrCreate, FileAccess.Write)) 
		{
			using(BinaryWriter bw = new BinaryWriter(fs))
			{
				Dictionary<string, List<string>>.Enumerator sjdEnum = standardJointData.GetEnumerator ();
	...
	void importBinarySJD()
	{
		...
		TextAsset ta = Resources.Load ("newStandardJointData") as TextAsset;
...
//		}
		...
		while (jviEnum.MoveNext ()) 
		{
            ...
            crd.name = key;
            crd.jointPosition = clothPis.jointPosition;
		...
		for (int i = 0; i < charPis.csms.Count; i++) 
		{
            ...
            if (jointName.Contains("Wrist") || jointName.Contains("Neck") || jointName.Contains("Ankle")
                || jointName.Contains("Head") || jointName.Contains("Foot"))
        ...
        for(int i = 0; i < charPis.csms.Count; i++)
        {
            names += charPis.csms[i].name + "\r\n";
        }

        Debug.Log(names);

        Debug.Log(charPis.csms.Count);
...
//	}
	...
	void calculateCustomRayCast(object arg)
	{
        ...
        string curName = crd.name;

        float fitClosedVal = 0.1f;

        //if (curName.Contains("Knee"))
        //{
        //    fitClosedVal = 0.02f;
        //}
        //else if(curName.Contains("LowLeg"))
        //{
        //    fitClosedVal = 0.01f;
        //}
        //else if (curName.Contains("Ankle"))
        //{
        //    fitClosedVal = 0.001f;
        //}
        ...
        //if (curName.Contains("Collar") || curName.Contains("Arm") || curName.Contains("Shoulder") || curName.Contains("Elbow") || curName.Contains("Wrist"))
        //{
        //    isPass = true;
        //}

        if (curName.Contains("UpLeg"))
        {
            string keepCurName = curName.Remove(2);

            maxTarY = crd.jointPosition[keepCurName + "UpLeg"].y;
            minTarY = crd.jointPosition[keepCurName + "Knee"].y;
        ...
        else if (curName.Contains("Knee"))
        {
            string keepCurName = curName.Remove(2);

            maxTarY = crd.jointPosition[keepCurName + "UpLeg"].y;
            minTarY = crd.jointPosition[keepCurName + "LowLeg"].y;
        ...
        else if (curName.Contains("LowLeg"))
        {
            ...
            maxTarY = crd.jointPosition[keepCurName + "Knee"].y;
            minTarY = crd.jointPosition[keepCurName + "Ankle"].y;
        ...
        else if (curName.Contains("Ankle"))
        {
            ...
            maxTarY = crd.jointPosition[keepCurName + "LowLeg"].y;
            minTarY = crd.jointPosition[keepCurName + "Foot"].y;

            maxFCV = 0.005f;
            minFCV = 0.0002f;

            //fitClosedVal = 0.0002f;
        }

        //if (curName.Contains("UpLeg") || curName.Contains("Knee") || curName.Contains("LowLeg") || curName.Contains("Ankle"))
        //{
        //    string keepCurName = curName.Remove(2);
        ...
        magnitude = minTarY - maxTarY;
        fcvMag = maxFCV - minFCV;
		...
		for (int i = 0; i < crd.csms.Count; i++) 
		{
            ...
            break;
            //if(curName.Contains("Collar") || curName.Contains("Shoulder") || curName.Contains("UpArm"))
            //{
            //    ;
            //}
            //else
		...
		for (int i = 0; i < clothVertices.Count; i++) 
		{
            ...
            if(magnitude != 0.0f)
            {
                if((maxTarY - position.y) > 0.0f)
                {
                    float curYPosVal = (minTarY - position.y);
                    if (curYPosVal < 0.0f)
                    {
                        fitClosedVal = minFCV + (fcvMag * (curYPosVal / magnitude));
                    }
                    else
                    {
                        fitClosedVal = minFCV;
...
//					break;
            ...
            if (isHit)
            {
                ...
                clothVertices[i] = (clothVertices[i] * 0.95f) + (position * 0.05f);

                //if (!isPass)
                //{
                //    clothVertices[i] = (clothVertices[i] * 0.9f) + (position * 0.1f);
                //}
		...
		Debug.Log (curName + " " + clothVertices.Count + " " + hitCount + " " + noHitIndexs.Count);
...
//				Vector3 jointPosition2 = pi.jointPosition [crdData[1]];
                ...
                if(!standardJointData.ContainsKey(minJointName))
                {
                    Debug.Log(minJointName);
                }
                ...
                //if(minJointName.Contains("Neck") || minJointName.Contains("Ankle") || minJointName.Contains("Wrist"))
                //{
                //    calcNormals.Add(normal);
                //}
                //else
                //{
                ...
                if(minJointName.Contains("Foot"))
                {
                    calcNormals.Add(Vector3.zero);
                }
                //else if (minJointName.Contains("Ankle"))
                //{
                //    calcNormals.Add(Vector3.zero);
                //}
                else
...
////			}
    ...
    PartitionInformation initialize(GameObject sourceObj)
    {
        ...
        //pi.jointPosition.Remove("L_Collar");
        //pi.jointPosition.Remove("R_Collar");

        Vector3 spine4 = pi.jointPosition["Neck"] + pi.jointPosition["Spine3"];
        spine4 /= 2.0f;
        pi.jointPosition.Add("Spine4", spine4);

        Vector3 LeftUpperLeg = pi.jointPosition["L_Hip"] + pi.jointPosition["L_Knee"];
        LeftUpperLeg /= 2.0f;

        Vector3 LeftLowerLeg = pi.jointPosition["L_Knee"] + pi.jointPosition["L_Ankle"];
        LeftLowerLeg /= 2.0f;

        pi.jointPosition.Add("L_UpLeg", LeftUpperLeg);
        pi.jointPosition.Add("L_LowLeg", LeftLowerLeg);

        Vector3 rightUpperLeg = pi.jointPosition["R_Hip"] + pi.jointPosition["R_Knee"];
        rightUpperLeg /= 2.0f;

        Vector3 rightLowerLeg = pi.jointPosition["R_Knee"] + pi.jointPosition["R_Ankle"];
        rightLowerLeg /= 2.0f;

        pi.jointPosition.Add("R_UpLeg", rightUpperLeg);
        pi.jointPosition.Add("R_LowLeg", rightLowerLeg);

        //float pelvisY = (pi.jointPosition["L_Hip"].y + pi.jointPosition["R_Hip"].y) / 2.0f;

        //Vector3 pelvis = pi.jointPosition["Pelvis"];
        //pelvisY = pelvis.y - pelvisY;
        //pelvis.y -= (pelvisY / 1.0f);
        //pelvis.z = pi.jointPosition["Spine1"].z;

        //pi.jointPosition["Pelvis"] = pelvis;

        pi.jointPosition.Remove("L_Hip");
        pi.jointPosition.Remove("R_Hip");

        Vector3 leftUpperArm = pi.jointPosition["L_Shoulder"] + pi.jointPosition["L_Elbow"];
        leftUpperArm /= 2.0f;

        Vector3 leftLowerArm = pi.jointPosition["L_Elbow"] + pi.jointPosition["L_Wrist"];
        leftLowerArm /= 2.0f;

        pi.jointPosition.Add("L_UpArm", leftUpperArm);
        pi.jointPosition.Add("L_LowArm", leftLowerArm);

        Vector3 rightUpperArm = pi.jointPosition["R_Shoulder"] + pi.jointPosition["R_Elbow"];
        rightUpperArm /= 2.0f;

        Vector3 rightLowerArm = pi.jointPosition["R_Elbow"] + pi.jointPosition["R_Wrist"];
        rightLowerArm /= 2.0f;

        pi.jointPosition.Add("R_UpArm", rightUpperArm);
        pi.jointPosition.Add("R_LowArm", rightLowerArm);

        pi.jointPosition["L_UpArm"] = (pi.jointPosition["L_UpArm"] + pi.jointPosition["L_Shoulder"]) / 2.0f;
        pi.jointPosition["R_UpArm"] = (pi.jointPosition["R_UpArm"] + pi.jointPosition["R_Shoulder"]) / 2.0f;

        pi.jointPosition["L_Collar"] = (pi.jointPosition["L_Collar"] + pi.jointPosition["L_Shoulder"]) / 2.0f;
        pi.jointPosition["R_Collar"] = (pi.jointPosition["R_Collar"] + pi.jointPosition["R_Shoulder"]) / 2.0f;

        Vector3 lShoulder = pi.jointPosition["L_Shoulder"];
        Vector3 rShoulder = pi.jointPosition["R_Shoulder"];

        //lShoulder.y = pi.jointPosition["Neck"].y;
        //rShoulder.y = pi.jointPosition["Neck"].y;

        float neckY = pi.jointPosition["Neck"].y;

        lShoulder.y += (Mathf.Abs(neckY - lShoulder.y) * 1.0f);
        rShoulder.y += (Mathf.Abs(neckY - rShoulder.y) * 1.0f);

        pi.jointPosition["L_Shoulder"] = lShoulder;
        pi.jointPosition["R_Shoulder"] = rShoulder;

        ///////////////////////////////////////////////////////
        /// finger

        pi.jointPosition.Remove("lindex0");
        pi.jointPosition.Remove("lindex1");
        pi.jointPosition.Remove("lindex2");
        pi.jointPosition.Remove("lmiddle0");
        pi.jointPosition.Remove("lmiddle1");
        pi.jointPosition.Remove("lmiddle2");
        pi.jointPosition.Remove("lpinky0");
        pi.jointPosition.Remove("lpinky1");
        pi.jointPosition.Remove("lpinky2");
        pi.jointPosition.Remove("lring0");
        pi.jointPosition.Remove("lring1");
        pi.jointPosition.Remove("lring2");
        pi.jointPosition.Remove("lthumb0");
        pi.jointPosition.Remove("lthumb1");
        pi.jointPosition.Remove("lthumb2");

        pi.jointPosition.Remove("rindex0");
        pi.jointPosition.Remove("rindex1");
        pi.jointPosition.Remove("rindex2");
        pi.jointPosition.Remove("rmiddle0");
        pi.jointPosition.Remove("rmiddle1");
        pi.jointPosition.Remove("rmiddle2");
        pi.jointPosition.Remove("rpinky0");
        pi.jointPosition.Remove("rpinky1");
        pi.jointPosition.Remove("rpinky2");
        pi.jointPosition.Remove("rring0");
        pi.jointPosition.Remove("rring1");
        pi.jointPosition.Remove("rring2");
        pi.jointPosition.Remove("rthumb0");
        pi.jointPosition.Remove("rthumb1");
        pi.jointPosition.Remove("rthumb2");
